...
- **후처리 문장형 재구성(결정적 보장)**: `NARRATIVE_SECTIONS`(추진배경·현황·현황분석·문제점·추진목표·기대효과·결론 등) 꼭지는 후처리(`_merge_narrative_paragraph`)에서 여러 문장을 **문장형 꼭지 2~3개**로 재구성함(문장 수 n: 3~5→2개, 6+→3개). 개요·일정·예산·협조 등 나열형은 항목 그대로 유지.
- **동일 라벨 반복 정리**: 나열형 섹션에서 `일시:`/`장소:`처럼 **모든 항목이 같은 라벨로 반복**되면(항목 채우기용 패딩) `_collapse_same_label`이 한 줄로 합침(예: `일시: A, B, C`). 서로 다른 라벨(총액:/설치: 등)은 유지.
  - 위 두 처리는 라우터 코드에서 동작하므로 **DB 프롬프트 override 상태이거나 GPT가 규칙을 안 지켜도 적용됨**
