...
1. `EMD_LIST` 공식 행정동명 직접 매칭
2. `EMD_ALIASES` 별칭 → 공식명 변환 (예: 호암동→호암직동, 목행동→목행용탄동)
3. Fallback 정규식 `[가-힣]{2,12}(?:읍|면|동)` 매칭 (비지명 한자어는 `EMD_FALLBACK_BLACKLIST`로 제외)

### 사고 유형 분류 우선순위

| 유형 코드 | 한글 | 대표 키워드 |
|-----------|------|------------|
| `rescue` | 수색·구조 | 실종, 수색, 인명구조 |
| `drainage` | 배수·맨홀·양수 | 맨홀, 역류, 배수, 양수 |
| `flood` | 침수·범람 | 침수, 범람, 수위, 하천 |
| `landslide` | 산사태·토사유출 | 산사태, 토사, 절개지 |
| `tree_fall` | 나무전도 | 나무전도, 수목전도, 나무 쓰러 |
| `sinkhole` | 싱크홀·노면파손 | 싱크홀, 함몰, 노면파손 |
| `road_control` | 도로통제 | 통제, 우회, 진입금지 |
| `facility` | 시설물 이상 | 시설물, 교량, 정전 |
