...
const QUICK_QUESTIONS = [
  ...
  '지방공무원 연가일수 규정 알려줘',
  '개인정보보호법에서 개인정보 제3자 제공 기준 알려줘',
  '소프트웨어사업 과업심의 대상사업 알려줘',
];
  ...
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      content:
        '안녕하세요! 국가법령 및 충주시 자치법규에 대해 답변드리는 AI 챗봇입니다. 법령·조례 구분 없이 궁금한 내용을 입력해 주세요.',
    },
  ]);
  ...
  const handleSubmit = async (question = input) => {
    ...
    try {
      ...
      const response = await api.post('/api/law-chatbot/ask', {
        question: question,
        // 백엔드 호환용으로 all 고정. 실제 백엔드는 통합검색으로 처리.
        search_scope: 'all',
      ...
      <div className="h-[calc(100vh-12rem)] flex flex-col animate-fadeIn">
                ...
                {msg.search_info &&
                  (msg.search_info.vector_count > 0 ||
                    ...
                    <div className="mt-1 flex gap-3 text-xs text-gray-400">
                      ...
                      {msg.search_info.api_count > 0 && (
                        <span>🏛️ 법령·자치법규 {msg.search_info.api_count}건</span>
                      )}
                      {msg.search_info.detail_count > 0 && (
        ...
        <div className="mt-4">
          ...
          <p className="text-center text-xs text-slate-500 mt-2">
            ※ AI 답변은 참고용이며, 정확한 법령 해석은 담당부서 및 국가법령정보센터에서 확인하시기 바랍니다.
          </p>
        </div>
      </div>
    </div>
