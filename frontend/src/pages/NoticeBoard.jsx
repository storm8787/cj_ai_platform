...
import { Bell, Plus, Eye, ChevronLeft, ChevronRight, Search } from 'lucide-react';
...
export default function NoticeBoard() {
  ...
  const fetchBoards = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('access_token');
      const response = await api.get(`/api/board/list/notice?page=${page}&limit=10`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm">중요한 안내사항을 확인하세요</p>
          </div>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-800">
              {filteredBoards.map((board) => (
                <tr
                  key={board.id}
                  onClick={() => navigate(`/board/notice/${board.id}`)}
