...
- 서술형(추진배경·현황·현황분석·문제점·상황개요 등): 짧게 끊지 말고 **2~3문장의 자연스러운 문장형**으로 서술(배경·원인·맥락을 이어서 설명, 문장 간 흐름 유지). 이 섹션은 content 배열에 **항목을 1~2개만** 넣고 각 항목을 2~3문장 문단으로 채움(한 문장씩 쪼개 여러 항목으로 나열 금지)
  (서술형 한 항목 예: "관내 출산율이 지속 하락하고 맞벌이 가정이 늘면서 돌봄 수요가 증가하고 있음. 그러나 공동육아나눔터 등 인프라는 부족하여 양육 부담 완화를 위한 지원 강화가 필요함.")
