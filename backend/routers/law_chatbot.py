...
async def _search_target(target: str, query: str, display: int = 10) -> List[Dict[str, Any]]:
    """
    target별 검색.
    1순위: MCP (korean-law-mcp CLI)
    2순위: law.go.kr 직접 API (MCP 실패 시 fallback)
    """
    # 1. MCP 시도
    try:
        if target == "law":
            mcp_results = await korean_law_mcp_service.search_law(
                query=query, target="law", display=display,
            )
        elif target == "ordin":
    ...
    # 2. law.go.kr 직접 API fallback
    direct = await _search_law_api_direct(query=query, targets=[target], display=display)
    if direct:
        print(f"[law-chatbot] 직접 API fallback 성공: target={target}, query={query}, count={len(direct)}")
    return direct
...
async def _get_full_text(candidate: Dict[str, Any]) -> str:
    ...
    try:
        if target == "law":
            text = await korean_law_mcp_service.get_law_text(mst=item_id, law_name=name)
        elif target == "ordin":
            text = await korean_law_mcp_service.get_ordinance_text(ordin_seq=item_id, ordinance_name=name)
        elif target == "admrul":
            text = await korean_law_mcp_service.get_admin_rule_text(admin_rule_id=item_id, admin_rule_name=name)
        else:
            text = ""
    ...
    # 2. law.go.kr 직접 API fallback
    if item_id:
        text = await _fetch_full_text_from_law_api(mst=item_id, target=target)
        if text:
            print(f"[law-chatbot] 직접 API 본문 조회 성공: target={target}, name={name}, chars={len(text)}")
            return text
...
async def _fetch_full_text_from_law_api(mst: str, target: str) -> str:
    ...
    try:
        ...
        print(f"[law-chatbot] 직접 API 본문 조회 실패: MST={mst}, error={e}")
        return ""
...
# =========================================================
# law.go.kr 직접 API (MCP 실패 시 fallback)
# =========================================================
...
def _parse_search_xml(xml_text: str, target: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for item in (
            ...
            r: Dict[str, Any] = {"type": target, "total_count": int(total or 0)}
            if target == "ordin":
...
async def _check_api_connection() -> Dict[str, Any]:
    result: Dict[str, Any] = {"mcp": {}, "law_api": {}, "connected": False}

    try:
        result["mcp"] = await korean_law_mcp_service.check_connection()
    except Exception as e:
        result["mcp"] = {"enabled": True, "connected": False, "reason": str(e)}

    oc = settings.LAW_API_OC
    if not oc:
        result["law_api"] = {"connected": False, "reason": "LAW_API_OC 미설정"}
    else:
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                text = resp.content.decode("utf-8", errors="ignore")
                result["law_api"] = {
                    "connected": not text.strip().startswith("<!DOCTYPE"),
                    "status_code": resp.status_code,
                }
        except Exception as e:
            result["law_api"] = {"connected": False, "reason": str(e)}

    result["connected"] = bool(
        result["mcp"].get("connected") or result["law_api"].get("connected")
    )
    return result