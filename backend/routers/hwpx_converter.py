...
# Outline level to Markdown heading
...
def _find_with_fallback(element, paths, namespaces):
    """Try multiple XPaths and return first match"""
    for path in paths:
        try:
            result = element.findall(path, namespaces)
            if result:
                return result
        except Exception:
            continue
...
def _extract_text_from_run(run_elem, ns):
    """Extract text from <run> element (handles fwSpace)"""
    texts = []
...
def _get_outline_level(para_elem, ns):
    """Check paragraph outline level (heading detection)"""
...
def _extract_images_from_hwpx(zf):
    """Extract image files from HWPX ZIP -> {key: (mime_type, base64_data)}"""
    images = {}
    ...
    for name in zf.namelist():
        ...
        if ("bindata" in lower or "media" in lower) and any(lower.endswith(ext) for ext in img_extensions):
            try:
                ...
                logger.info(f"[hwpx-converter] image extracted: {name} ({len(data)} bytes)")
            except Exception as e:
                logger.warning(f"[hwpx-converter] image extract failed: {name} - {e}")

        return images

def _find_image_ref(para_elem, ns):
    """Find image reference (binDataId or href) in paragraph"""
...
def _parse_table(tbl_elem, ns):
    """Convert <tbl> element to Markdown table"""
    rows_data = []
    max_cols = 0
    ...
    def _find_section_files(zf):
        """Find section XML files in HWPX (sorted)"""
        section_files = []
        for name in zf.namelist():
            lower = name.lower()
...
def _convert_section_to_markdown(section_xml, images, image_mode="inline", image_counter=None):
    """Convert section XML to Markdown text"""
    ...
    try:
        ...
        logger.warning(f"[hwpx-converter] XML parse error: {e}")
        return ""
    ...
    for elem in root.iter():
        ...
        # Skip paragraphs inside table cells (already handled)
        ...
        if outline_level and outline_level in OUTLINE_LEVEL_MAP:
            ...
            # Bold detection (short bold paragraph = subheading)
            is_all_bold = False
            runs = list(elem.iter())
...
def _clean_markdown(md_text):
    """Post-process markdown (remove extra blank lines)"""
    lines = md_text.split("\n")
    cleaned = []
    prev_empty = False
...
def convert_hwpx_to_markdown(file_bytes, image_mode="inline", filename="document"):
    ...
    Args:
        file_bytes: HWPX file bytes
        image_mode: "inline" (base64) or "separate" (file references)
        filename: original filename

    Returns:
        dict with markdown, images, stats
    """

    try:
        zf = zipfile.ZipFile(io.BytesIO(file_bytes), "r")
    except zipfile.BadZipFile:
        raise ValueError("Not a valid HWPX file. HWPX must be ZIP format.")

    # 1. Extract images
    images = _extract_images_from_hwpx(zf)
    logger.info(f"[hwpx-converter] {len(images)} images extracted")
    ...
    logger.info(f"[hwpx-converter] section files: {section_files}")
    ...
    for sf in section_files:
        try:
            ...
            section_md = _convert_section_to_markdown(
                section_xml, images, image_mode, image_counter
            )
            if section_md.strip():
                all_md_parts.append(section_md)
                table_count += section_md.count("| ---")
        except Exception as e:
            logger.warning(f"[hwpx-converter] section convert failed: {sf} - {e}")
...
async def convert_hwpx(
    file: UploadFile = File(..., description="HWPX file"),
    image_mode: str = Form("inline", description="Image mode: inline or separate"),
):
    """Convert HWPX to Markdown and return JSON response"""
    ...
    try:
        result = convert_hwpx_to_markdown(
        ...
        logger.error(f"[hwpx-converter] convert error: {e}", exc_info=True)
...
async def convert_hwpx_download(
    ...
    """Convert HWPX to Markdown and return .md file download"""
