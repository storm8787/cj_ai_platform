...
# '차단/출입통제/통제 완료'는 통제 조치를 끝낸 것 → 발생(미착수) 아님 → completed로 분류
#   (해제·재개는 위 closed 규칙이 먼저 처리하므로 여기서는 통제 착수 완료만 잡힘)
STATUS_RULES = [
    (re.compile(r"해제(?!\s*예정)|통행\s*재개(?!\s*가능)|개통|상황\s*종료"), "closed"),
    (re.compile(r"복구 완료|처리 완료|긴급조치 완료|제거 완료|응급복구 완료|양수 작업 완료|조치 완료|조치완료|완료했습니다|완료하였습니다|제설 완료|살포 완료|염화칼슘 살포|출입\s*통제 완료|출입통제\s*완료|통행\s*차단 완료|차단 완료|통제 완료"), "completed"),
