...
def generate_daily_report(report_date: str, incidents: List[Dict]) -> Dict:
    ...
    major_items = []
    for idx, incident in enumerate(incidents[:10], start=1):
        major_items.append(
            f"  ◦ {incident.get('emd') or ''} {incident.get('location_raw') or ''} / "
            f"{INCIDENT_LABELS.get(incident.get('incident_type'), incident.get('incident_type'))} / "
            f"{STATUS_LABELS.get(incident.get('status'), incident.get('status'))} / "
            f"{incident.get('summary') or ''}"
        )

    type_lines = []
    for code, count in incident_counter.most_common():
        type_lines.append(f"  ◦ {INCIDENT_LABELS.get(code, code)} : {count}건")

    status_lines = []
    for code, count in status_counter.most_common():
        status_lines.append(f"  ◦ {STATUS_LABELS.get(code, code)} : {count}건")

    report_text = (
        f"1. 재난상황 총괄\n"
        f"  ◦ {report_date} 기준 카카오톡 상황보고 분석 결과, 총 {len(incidents)}건의 유효 사건이 확인되었음\n"
        f"  ◦ 주요 유형은 {', '.join([f'{INCIDENT_LABELS.get(k, k)} {v}건' for k, v in incident_counter.most_common(3)])}으로 분석되었음\n\n"
        f"2. 유형별 발생현황\n"
    ...
    summary_text = f"{report_date} 기준 총 {len(incidents)}건 분석, 완료 {status_counter.get('completed', 0) + status_counter.get('closed', 0)}건, 조치중 {status_counter.get('in_progress', 0)}건"
